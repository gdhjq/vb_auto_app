...
pg_log_dir=/data/pgdata/pg1921/data/log
...
echo "----->>>---->>>  获取错误日志信息: "
cat postgresql-2019-12-2*.log | grep -E "^[0-9]" | grep -E "WARNING|ERROR|FATAL|PANIC" |sort -rn
echo "建议: "
echo "    参考 http://www.postgresql.org/docs/current/static/errcodes-appendix.html ."
echo -e "\n"

echo "----->>>---->>>  获取连接请求情况: "
find . -name "postgresql-2019-12-2*.log" -type f -mtime -28 -exec grep "connection authorized" {} +|sort|uniq |sort -n -r
echo "建议: "
echo "    连接请求非常多时, 请考虑应用层使用连接池, 或者使用pgbouncer连接池. "
echo -e "\n"

echo "----->>>---->>>  获取认证失败情况: "
find . -name "postgresql-2019-12-2*.log" -type f -mtime -28 -exec grep "password authentication failed" {} +|sort|uniq|sort -n -r
echo "建议: "
...
echo "----->>>---->>>  慢查询统计: "
cat postgresql-2019-12-2*.log|grep "duration:"|grep -v "plan:"|sort|uniq -c|sort -rn
echo "建议: "
...
echo "----->>>---->>>  慢查询分布头10条的执行时间, ms: "
 cat postgresql-2019-12-2*.log|grep "duration:"|grep -v "plan:"  |sort -k 10 -n|tail -10
echo ""
echo "----->>>---->>>  慢查询分布尾10条的执行时间, ms: "
 cat postgresql-2019-12-2*.log|grep "duration:"|grep -v "plan:"  |sort -k 10 -n|head -10
echo -e "\n"
...
echo ""
echo "    检查 *.csv :  "
cat postgresql-2019-12-2*.log | grep -E "^[0-9]" | grep -i -r -E "role|group|user" |grep -i "password"|grep -i -E "create|alter"
echo ""
