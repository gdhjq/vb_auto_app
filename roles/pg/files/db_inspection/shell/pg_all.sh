...
export PGDATA=/home/postgres/pgdata95
export PGHOME=/usr/local/pg95
...
pg_log_dir_base=`cat  "$PGDATA/postgresql.base.conf"|awk -F "#" '{print $1}'|grep log_directory|awk -F "=" 'END{print $2}' |sed $'s/\'//g' | sed 's/[[:space:]]//g'`
...
psql --pset=pager=off -q -x -c "$top5_sql"
echo "建议: "
...
echo ""
echo "    检查 *.csv :  "
cat postgresql-${this_month}*.log | grep -E "^[0-9]" | grep -i -r -E "role|group|user" |grep -i "password"|grep -i -E "create|alter"
echo ""
