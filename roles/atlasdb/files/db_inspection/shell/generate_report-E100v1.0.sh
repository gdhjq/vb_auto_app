...
export PGPASSWORD=Vast@123
export PGDATA=/data/adbdata/
export PGHOME=/home/atlasdb/adbhome
...
# 获取postgresql日志目录
pg_log_dir=`grep '^\ *[a-z]' $PGDATA/atlasdb.conf|awk -F "#" '{print $1}'|grep log_directory|awk -F "=" '{print $2}'`
...
echo "----->>>---->>>  获取错误日志信息: "
cat postgresql-$this_month-*.log | grep -E "^[0-9]" | grep -E "WARNING|ERROR|FATAL|PANIC" |sort -rn
echo "建议: "
echo "    参考 http://www.postgresql.org/docs/current/static/errcodes-appendix.html ."
echo -e "\n"

echo "----->>>---->>>  获取连接请求情况: "
find . -name "postgresql-$this_month-*.log" -type f -mtime -28 -exec grep "connection authorized" {} +|sort|uniq |sort -n -r
echo "建议: "
echo "    连接请求非常多时, 请考虑应用层使用连接池, 或者使用pgbouncer连接池. "
echo -e "\n"

echo "----->>>---->>>  获取认证失败情况: "
find . -name "postgresql-$this_month-*.log" -type f -mtime -28 -exec grep "password authentication failed" {} +|sort|uniq|sort -n -r
echo "建议: "
...
echo "----->>>---->>>  慢查询统计: "
cat postgresql-$this_month-*.log|grep "duration:"|grep -v "plan:"|sort|uniq -c|sort -rn
echo "建议: "
...
echo "----->>>---->>>  慢查询分布头10条的执行时间, ms: "
 cat postgresql-$this_month-*.log|grep "duration:"|grep -v "plan:"  |sort -k 10 -n|tail -10
echo ""
echo "----->>>---->>>  慢查询分布尾10条的执行时间, ms: "
 cat postgresql-$this_month-*.log|grep "duration:"|grep -v "plan:"  |sort -k 10 -n|head -10
echo -e "\n"
...
psql -d $db --pset=pager=off -q -x -c "SELECT
        ...
        CASE WHEN ipages < iotta THEN 0 ELSE ipages::bigint - iotta END AS wastedipages,
        CASE WHEN ipages < iotta THEN 0 ELSE bs*(ipages-iotta) END AS wastedibytes,
        CASE WHEN ipages < iotta THEN pg_size_pretty(0) ELSE pg_size_pretty((bs*(ipages-iotta))::bigint) 
                  END AS wastedisize
      FROM (
        SELECT
...
echo "----->>>---->>>  归档统计信息: "
psql --pset=pager=off -q -c 'select pg_walfile_name(pg_current_wal_lsn()) now_xlog, * from pg_stat_archiver'
echo "建议: "
echo "    如果当前的XLOG文件和最后一个归档失败的XLOG文件之间相差很多个文件, 建议尽快排查归档失败的原因, 以便修复, 否则pg_xlog目录可能会撑爆. "
echo -e "\n"

echo "----->>>---->>>  流复制统计信息: "
psql --pset=pager=off -q -x -c 'select pg_wal_lsn_diff(pg_current_wal_lsn(),flush_lsn), * from pg_stat_replication'
echo "建议: "
echo "    关注流复制的延迟, 如果延迟非常大, 建议排查网络带宽, 以及本地读xlog的性能, 远程写xlog的性能. "
echo -e "\n"

echo "----->>>---->>>  流复制插槽: "
psql --pset=pager=off -q -c 'select pg_wal_lsn_diff(pg_current_wal_lsn(),restart_lsn), * from pg_replication_slots'
echo "建议: "
...
echo ""
echo "    检查 *.csv :  "
cat postgresql-$this_month-1*.log | grep -E "^[0-9]" | grep -i -r -E "role|group|user" |grep -i "password"|grep -i -E "create|alter"
echo ""
